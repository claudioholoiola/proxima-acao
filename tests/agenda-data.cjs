...
const result = vm.runInContext(`(() => {
  ...
  const dayPlan=buildDayPlanProposals(plannerDb,new Date("2026-07-21T09:00:00"),0),plannedCoverage=dayPlan.proposals.reduce((sum,item)=>sum+minutes(item.endTime)-minutes(item.startTime),0),overlapsFixed=dayPlan.proposals.some(item=>minutes(item.startTime)<660&&600<minutes(item.endTime));
