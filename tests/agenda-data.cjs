...
const result = vm.runInContext(`(() => {
  ...
  const plannerReference=new Date("2026-07-21T09:00:00"),casaBeforePlan=responsibilityStates(plannerDb,plannerReference).find(item=>item.area==="casa"),dayPlan=buildDayPlanProposals(plannerDb,plannerReference,0),plannedCoverage=dayPlan.proposals.reduce((sum,item)=>sum+minutes(item.endTime)-minutes(item.startTime),0),overlapsFixed=dayPlan.proposals.some(item=>minutes(item.startTime)<660&&600<minutes(item.endTime)),plannedAreas=[...new Set(dayPlan.proposals.filter(item=>item.blockType!=="margin").map(item=>item.responsibilityArea))],orderedProposals=[...dayPlan.proposals].sort((a,b)=>minutes(a.startTime)-minutes(b.startTime)),adjacentMargins=orderedProposals.some((item,index)=>item.blockType==="margin"&&orderedProposals[index+1]?.blockType==="margin"&&item.endTime===orderedProposals[index+1].startTime);
